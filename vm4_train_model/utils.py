...
class Parser:
    ...
    def get_tickers_codes(self):
        with open('../vm1_flask/services/flask_app/static/support_files/tickers.json', 'r+') as f:
            tickers = json.load(f)
        return tickers
...
class FeatureCreator:
    ...
    def trend(self, df):
        for hour in self.hours_trens:
            ...
            for i, _ in enumerate(df['close']):
                ...
                idx_end = min(idx_end, len(df['close']) - 1)
    ...
    def dem(self, df):
        ...
        for hour in self.hours_ma:
            ...
            df[f'demark_{hour}'] = ma_demax/(ma_demax + ma_demin + self.eps)
        return df
