...
DATF = 1
AT = 0
ISS_MOEX_URL = 'https://iss.moex.com/iss/engines/stock/markets/shares/securities/'
ISS_MOEX_URL_SUFFIX = '/candles.json?iss.meta=off&iss.reverse=true&interval=60&from='


class Parser:
    def __init__(self, ticker, from_, to_=None, ticker_code_path='tickers.json'):
        self.ticker = ticker
        self.from_ = from_
        self.to_ = to_
        self.last_date_time = pd.Timestamp(from_format(self.to_, 'DD.MM.YYYY HH:00:00').in_timezone('Europe/Moscow'))
        self.ticker_code_path = ticker_code_path
        if self.to_:
    ...
    def get_tickers_codes(self):
        with open(self.ticker_code_path, 'r+') as f:
            tickers = json.load(f)
        return tickers
    ...
    def get_dates_by_year(self, from_, to_):
        ...
        new_from_ = from_
        while True:
            new_to_ = (pd.Timestamp(new_from_)+pd.Timedelta(value=365, unit='D')).strftime('%d.%m.%Y %H:00:00')
    ...
    def get_url_period(self, ticker, start, end):
        ...
        start_date = datetime.strptime(start, '%d.%m.%Y %H:00:00')
        end_date = datetime.strptime(end, '%d.%m.%Y %H:00:00')
    ...
    def get_all_data_as_df(self, all_data):
        for h_data in all_data:
            ...
            if self.last_date_time < date_time:
                break
            if date_time.hour >= 10 and date_time.hour < 20:
...
class FeatureCreator:
    ...
    def so(self, df):
        for hour in self.hours_ma:
            ...
            (df['high'].rolling(hour).max() - df['low'].rolling(hour).min() + self.eps) * 100
...
class PostgresLoader:
    ...
    def write_to_sql(self, df, ticker):
        ...
        with connect_db.cursor() as cursor:
            ...
            sql_insert = 'INSERT INTO "%s" VALUES %%s' % (ticker)
            print(sql_insert)
