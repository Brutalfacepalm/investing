...
class Parser:
    ...
    def get_tickers_codes(self):
        with open('static/support_files/tickers.json', 'r+') as f:
            tickers = json.load(f)
        return tickers
...
class PostgresLoader:
    ...
    def write_to_sql(self, df, ticker):
        ...
        cursor = conn1.cursor()
        print('cursor')
        sql = 'CREATE TABLE IF NOT EXISTS "{}" (time date, open float, high float, low float, close float, volume int);'.format(ticker)
        print(sql)
        cursor.execute(sql)
        print('execute')
        df.to_sql(f'{ticker}', conn, if_exists='replace')
        print('to_sql')
        # #
        conn1.commit()
        print('commit')
    ...
    def read_from_sql(self, ticker):
        ...
        #     if not cnt:
        #         break
        tohlcv = list(zip(*cursor.fetchall()))
        conn1.commit()
        conn1.close()
